...
export class PaymentSettingComponent  implements OnInit {
  ...
  creditCardSetup() {
    ...
    let cardNumber = this.creditCardForm.get('cardNumber')?.value;

      // Remove hyphens from the phone number
      this.creditCardForm.patchValue({
        cardNumber:cardNumber.replace(/-/g, '')
      })
      // cardNumber = cardNumbers.replace(/-/g, '');

    console.log("this.formVal",this.formVal)
    this.userSetting.creditCardSetup(this.formVal)
      .subscribe(
        (res: any) => {
          if (res) {
            this.toastr.success('Payment Settings Save  Successfully"!', 'Success!',{timeOut: 2000});
            // this.getCreditCardInfo();
            // this.reset();
          } else {
            this.toastr.error('Payment Settings Not Save "!', 'Error!',{timeOut: 2000});
  ...
  getProfile() {
    this.userSetting.getProfile(this.userId).subscribe((response: any) => {
      if (response) {
        this.getUserDetailsItem = response as any[];
        let data = this.getUserDetailsItem[0];
        this.creditCardForm.patchValue(this.getUserDetailsItem)
  ...
  getProfileBusiness() {
    this.userSetting.getProfileBusiness(this.userId).subscribe((response: any) => {
      if (response) {
        this.getUserDetailsItem = response as any[];
        let data = this.getUserDetailsItem[0];
  ...
  onToggleChanged(field: string, event: CustomEvent) {
    ...
    if (event.detail.checked == 1 && this.signUpAs== 'Rider' ) {
      this.getProfile();
    } else if (event.detail.checked == 1 && this.signUpAs== 'Driver' ) {
      this.getProfileDriver();
    } else if (event.detail.checked == 1 && this.signUpAs== 'Business' ) {
